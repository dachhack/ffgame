...
| 🪂 **Bye Steal** | 55 | empty slot | Field one of your **bye-week** players in an open slot for a flat projected score (**capped at 16**). |
...
| ⚔️ **Rivalry** | 70 | window (blind) | For every slot where the opponent fields the **same position** as you, **siphon 30%** of their slot score to you at window's end. | Whiffs entirely if they don't mirror your position — a bet on how they build the window (and they can dodge it). |
...
|---|---|---|---|---|
| ⚡ **Surge** | 55 | your slot | Everything the slot scores for the next **10 game-minutes counts double**. | The moment your player goes **HOT** — and if nothing is hot by ~30:00, fire anyway. Late beats early (the ×2 window covers the dense late-game scripts), and the playtester puts perfect timing at nearly **double** the value of a blind early fire. |
| 🧊 **Cold Snap** | 60 | opponent slot | **Freeze ALL** of that player's scoring — points and drip — for 10 game-minutes. | The moment their player goes **HOT** — else late (~30:00). Freezing a hot drip near the end leaves no time to rebuild it. |
| 🔥 **Napalm** | 60 | opponent slot | For 10 game-minutes, any time their drip runs **HOT it BURNS** — the hot accrual goes **negative** and bleeds their bank instead of doubling it. | Strictly on **their hot streak** — it does nothing while they stay cool, and timing matters more here than for any other play (perfect timing measures **8×** a blind early fire). |
