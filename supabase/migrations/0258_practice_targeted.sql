-- 0258: targeted power-ups WORK in practice weeks.
--
-- 0086's entitlement gate derives purchases from coin_ledger 'spend:<id>'
-- rows — and practice weeks, by 0110's deliberate invariant, never write the
-- ledger (adjust_wallet refuses them; probe 5g pins it). So every targeted
-- apply on a practice board counted 0 bought and returned 'not owned', and
-- the web swallowed that into a console warn: the local board drew the play,
-- the official resolver never saw it, and the two disagreed for the rest of
-- the week. v0.372.1 makes the client refusal LOUD; this makes the practice
-- refusal WRONG no longer.
--
-- The fix: practice weeks SKIP the ledger entitlement. Practice runs the
-- throwaway 120/week purse (0115) — the gate exists to stop free plays in the
-- REAL economy, and a practice apply can't touch that economy at all. The
-- per-play caps (one DoN, 6 per slot-list, one fire per slot) still bound a
-- sandbox week. Body is 0086's verbatim; only the gate's condition changed.
