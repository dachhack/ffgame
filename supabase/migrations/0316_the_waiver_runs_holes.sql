-- 0316: THE WAIVER RUN'S HOLES (v0.434.6). Founder: "let's check the waiver
-- system for holes." Four found reading the run end to end; three closed
-- here, the fourth (dropping a player whose game has started) left as the
-- founder's call and written up in STATUS.
--
...
--      every claim on him competes in one run.
--   3. A CLAIM WHOSE DROP HAD ALREADY LEFT THE ROSTER LOST, even with a place
--      open — an earlier claim of the same seat had spent him. It goes
--      through without the drop when the seat has room, and loses only when
--      it would need the drop to make room; the claim's row and the report
--      say which.
--   Also: "outbid" only when a claim in this run took the player; one signed
--   off free agency while the claim sat was simply "player taken".
--
-- Bodies are 0290's (process_waivers) and 0291's (submit_waiver_claim), with
-- the marked additions.
...
begin
  ...
  loop
    ...
    update waiver_claim set status = 'won', processed_at = now(),
      note = case when c.drop_slug is null and waiver_claim.drop_slug is not null then 'drop had already left the roster' else note end,
