-- 0167 · Phase-2 SPECIAL TEAMS + TEAM BRACKETS: KR/PR split, blocked kicks,
-- points/yardage-allowed brackets, punt-row groundwork.
--
-- The founder's "let's build 2" (docs/play-feed-enrichment-scope.md). Additive
-- like 0166: new kinds (punt / blk / pa / ya) are invisible to every legacy
-- scorer, the `rk` return flag is absent on legacy rows (split knobs score
-- nothing there — combined RETURN YD still works everywhere), and the
-- points-allowed bracket defaults only ever meet `pa` rows emitted at game
-- FINAL, so no historical total moves.

alter table live_play add column if not exists rk text;

-- ── Scoring whitelist widens with the Phase-2 knobs ─────────────────────────
-- New clamp class rate_keys (−1..1, 3 decimals): per-point / per-yard-allowed
-- rates are small and legitimately negative — the yard clamp (0..1) can't hold
-- them and the event clamp's 0.1 rounding would erase them.
...
declare
  ...
  event_keys text[] := array['passTd', 'int', 'pass300', 'pass400',
                             ...
                             'sack', 'dstInt', 'fumRec', 'dstTd', 'safety', 'dstBlk',
