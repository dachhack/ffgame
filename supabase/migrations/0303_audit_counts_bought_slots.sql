-- 0303: THE AUDIT COUNTS A BOUGHT SLOT (v0.430.1).
--
-- Founder, reading the week's audit: "I think I added a slot in one of my
-- drip leagues but they both say x/9 slots filled."
--
-- 0302 judged every drip seat against the league's base count — the week's
-- windows (week_slot_count), never fewer than eight. A drip league has no
-- league-wide way to add a slot; a seat adds one by BUYING it (0027
-- buy_extra_slot), which lands in that seat's applied_state.payload_json.extra
-- for that matchup, and enforce_slot_cap has always allowed that seat exactly
-- base + extra. The audit now reads the same number: a seat's expected is the
-- league's base plus its own extras on its matchup, so a bought slot that was
-- filled reads 10/10, and one left empty reads 9/10 with one EMPTY — instead
-- of 10/9 and 9/9, which is what "x/9" was. Classic leagues are untouched
-- (their spot list is the cap; no extras exist there). Same function, same
-- signature, same shape; only `expected` per seat (and its roll-ups) moves.
...
begin
  ...
  expected as (
    select lg.id as league_id, lg.game_mode,
  ...
  team_rows as (
    select s.league_id, s.roster_id, s.team, s.kind, s.controller, s.enrolled,
           ...
           -- The league's base count PLUS this seat's own extra slots (0027
           -- buy_extra_slot → applied_state.payload_json.extra, per matchup):
           -- a seat that bought a slot is judged against the board it had.
           ex.n + case when ex.game_mode = 'classic' then 0 else coalesce(
             (select (a.payload_json ->> 'extra')::int from applied_state a
                join side sd on sd.matchup_id = a.matchup_id
               where sd.league_id = s.league_id and sd.roster_id = s.roster_id
                 and a.app_user_id = s.seat_uid limit 1), 0) end as expected,
