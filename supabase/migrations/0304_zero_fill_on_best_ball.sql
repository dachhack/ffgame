-- 0304: THE ZERO-FILL RULE RIDES ON BEST-BALL SPOTS.
--
-- Founder, on a classic golf league: "best ball in golf should slot 0 players
-- over players with more than 10 points." That sentence only means something
-- if the spot BANKS 10 for a zero — the zero-fill rule — and since v0.303.0
-- the builder has refused to put that rule on a best-ball spot: "it fills
-- itself, so it is never unfilled." True of the UNFILLED half of the rule,
-- and blind to the other half: whoever the fill seats can still score
-- nothing (a bye, a late scratch, a bench role), and in a golf league that
-- zero was the best score on the board with nothing to price it.
--
-- So the refusal goes. Everything downstream already handles a best-ball
-- spot with a fill — the resolver and the board walk every slot when they
-- pay it, and core's classicSlotsFromSpec / bestballFillBy now carry and
-- rank by it (v0.430.2). This is 0201's set_league_classic_slots verbatim
-- minus the four refusing lines.
