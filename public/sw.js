...
self.addEventListener('push', (e) => {
  ...
  e.waitUntil(self.registration.showNotification(p.title || 'Drip Fantasy', {
    ...
    // The BADGE is Android's status-bar glyph and it keeps only the ALPHA
    // channel — a full-colour icon arrives as a solid square and the system
    // draws its own robot instead, which is what this used to do. The
    // silhouette is generated by scripts/gen-notification-icon.py, the same
    // asset the app ships as its small icon, so both surfaces wear one mark.
    badge: new URL('./icons/pwa/badge-96.png', ROOT).href,
    data: p.data || {},
