...
export interface LivePlayRow { player_slug: string; c: number; t: number | null; pid: number | null; k: string; y: number; td: number; ca: number; tg: number; to: number | null; fd?: number | null; cp?: number | null; ic?: number | null; sk?: number | null; rk?: string | null; }
export async function weekLivePlays(week: number): Promise<LivePlayRow[]> {
  ...
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await (await client()).from('live_play')
      .select('player_slug, c, t, pid, k, y, td, ca, tg, to, fd, cp, ic, sk, rk')
      .eq('week', week)
