...
export interface RawPlay {
  ...
  gid?: string;     // the source GAME id (live_play.game_id) — the exact
                    // membership key when present (v0.369.0); pid/clock stay
                    // the fallback for baked and sim data that lack it.
  kind: RealPlayKind;
  yards: number;
  td: boolean;
...
function scorePlay(play: RawPlay, pos: Pos, metricId: string, hot: boolean): number {
  ...
  if (pos === 'QB') {
    ...
    // 0.2/yd + 8/TD (founder's balance call, v0.193.2 — was 0.1 + 6): a
    // scrambler's 50-yd night read ~8 pts against pass-flat's ~16, so the
    // metric was never a real pick. At 0.2 a real rushing-QB night (85 yds
    // + a TD) lands ~25 — a genuine boom — while a 30-yd night is still ~6,
    // below pass-flat: a deliberate call FOR a running QB, not a free
    // upgrade. The TD spike keeps the flat family's boom-bust identity.
    if (metricId === 'rush') return play.kind === 'rush' ? play.yards * 0.2 * A.ydMult + tdPts(8) : 0;
  }
...
export function rawPlaysFrom(ps: RealPlay[]): RawPlay[] {
  return ps
    .map((p) => ({
      clock: p.c, t: p.t, pid: p.pid, ...(p.gid ? { gid: p.gid } : {}), kind: p.k, yards: p.y, td: !!p.td, catch: !!p.ca, target: !!p.tg, turnover: !!p.to,
