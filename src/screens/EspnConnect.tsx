...
import { getProvider, espnAuth } from '../data/providers';

// Connect an ESPN league. ESPN has no public username lookup, so this is
// league-id-centric: the user pastes their league id (+ season) and, for a
// PRIVATE league, their espn_s2 + SWID cookies. The build goes through the
// espn-league Supabase proxy; on success we load the sim and jump to the hub.
export function EspnConnect() {
  ...
  const submit = async () => {
    ...
    if (!id || busy) return;
    if (!/^\d+$/.test(id)) { setErr('The ESPN league id is the number in the league URL (leagueId=…).'); return; }
    setBusy(true); setErr(null);
    try {
      const { built, youTeamId } = await getProvider('espn').buildLeague(id, '', setNote, {
        addKdst,
        auth: espnAuth({ swid: swid.trim(), s2: s2.trim(), season: season.trim() }),
      });
      loadSimLeague(built, youTeamId);
      navigate({ name: 'hub' });
    } catch (e) {
      setErr(e instanceof Error ? e.message : 'Could not load that ESPN league.');
      setBusy(false);
    }
  };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid var(--bd)', flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          ...
          <span className="grotesk" style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>Connect an ESPN league</span>
        </div>
        <SiteSettings />
      </header>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--bd)', borderLeft: '3px solid var(--you)', borderRadius: 8, padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ flex: 2 }}>
                ...
                <input value={leagueId} autoFocus onChange={(e) => { setLeagueId(e.target.value); setErr(null); }} onKeyDown={(e) => { if (e.key === 'Enter') submit(); }} placeholder="e.g. 123456" inputMode="numeric" style={field} />
              </div>
              <div style={{ flex: 1 }}>
