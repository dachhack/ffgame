...
import { buildMatchup, defaultLineup, aiLineup, slotKey, slotsFor, windowPools, byePlayers, banksAtClock, type ResolvedSlot } from '@drip/core/engine/matchup';
import { YOU_TEAM_ID, gameForTeam, getTeam } from '@drip/core/data/league';
...
import { getSession, demoCardTheme } from '@drip/core/data/liveApi';
...
import type { Pick, Player, WindowId } from '@drip/core/types';
...
export function DemoBoard() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          {phase === 'setup' && narrow && (
            <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setRosterOpen((o) => (o.you ? { ...o, you: false } : { you: true, their: false }))} className={promptIdx === 0 && !rosterOpen.you ? 'mono guide-ring' : 'mono'} style={{ flex: 1, minWidth: 0, whiteSpace: 'nowrap', textAlign: 'center', fontSize: fs(9.5), fontWeight: 700, letterSpacing: '0.08em', padding: '8px', borderRadius: 4, background: 'var(--surface)', border: `1px solid ${rosterOpen.you ? 'var(--you)' : 'var(--bd)'}`, color: rosterOpen.you ? 'var(--you)' : 'var(--dim)', cursor: 'pointer' }}>{rosterOpen.you ? '▾' : '▸'} YOUR ROSTER</button>
                <button onClick={() => setRosterOpen((o) => (o.their ? { ...o, their: false } : { you: false, their: true }))} className="mono" style={{ flex: 1, minWidth: 0, whiteSpace: 'nowrap', textAlign: 'center', fontSize: fs(9.5), fontWeight: 700, letterSpacing: '0.08em', padding: '8px', borderRadius: 4, background: 'var(--surface)', border: `1px solid ${rosterOpen.their ? 'var(--opp)' : 'var(--bd)'}`, color: rosterOpen.their ? 'var(--opp)' : 'var(--dim)', cursor: 'pointer' }}>{rosterOpen.their ? '▾' : '▸'} OPPONENT ROSTER</button>
              </div>
      ...
      {requesting && <RequestCodeModal initialPlatform={sleeperUser ? 'Sleeper' : ''} initialNote={requestNote} onClose={() => { setRequesting(false); setRequestNote(undefined); }} />}
