...
export type Route =
  ...
  // v0.288.1 'leaguehome' to one league's hub — and since v0.356.11 'team' and
  // 'draft' to that league's other rooms, so the matchup board's room bar can
  // reach them. All three need `leagueId` too,
...
interface Store {
  ...
  exitSimLeague: () => void;
  /** Bumps each time a fresh live injury report lands. Nothing reads the value —
  ...
  commishVer: number;
  /** The live league's scoring adjustments (0143) — installed into the engine
   *  module; exposed here for the banner chip + editor. Null off live boards. */
  liveScoring: (LeagueScoring & { canEdit: boolean }) | null;
  /** Re-pull the note + flags + scoring now — call after a commissioner edit
   *  so the board reflects it without waiting out the poll. */
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // served by the baked report, so loading over it would be wrong, not merely
  // redundant. `injuryVer` bumps when a report lands: `injuryFor` reads a module
  // cache (it has to be synchronous — the engine calls it), so nothing would
  // re-render on its own; threading the counter through the store re-renders the
  // consumer tree and the badges with it.
  //
  // It re-polls because designations MOVE, and they move when it matters most:
  ...
  // Slow cadence; the worker only refreshes hourly on game days.
  const [injuryVer, setInjuryVer] = useState(0);
  useEffect(() => {
    if (!liveCtx) return;
    let alive = true;
    const load = async () => {
      if (document.hidden) return; // don't poll in a background tab
      const n = await loadLiveInjuries(liveCtx.week); // swallows its own failures
      if (alive && n) setInjuryVer((v) => v + 1);
    };
    void load();
  ...
  const [liveScoring, setLiveScoring] = useState<(LeagueScoring & { canEdit: boolean }) | null>(null);
  const reloadCommish = async () => {
    ...
    const [n, f, sc] = await Promise.all([
      ...
      leagueScoringGet(ctx.leagueId).catch(() => null),
    ]);
    ...
    if (sc && sc.ok) {
      ...
      setLeagueScoring(knobs);                       // the engine resolves with these
      setLiveScoring({ ...knobs, canEdit: !!sc.can_edit });
      setCommishVer((v) => v + 1);                   // live board re-derives scores
    }
  };
  useEffect(() => {
    if (!liveCtx) { setLiveNote(null); setLiveScoring(null); clearLeagueScoring(); return; }
    let alive = true;
