...
was already shrunk (R8, resource shrinking, compressed native libraries), so
the target was the JS bundle, which ships uncompressed so it loads fast. The
bundle went from 7.80 MB to 5.86 MB, and images dropped another ~430 KB.

- **The 2025 season bake is gone from the app (1.57 MB).** Founder: "We only
  needed to load 2025 data for playtesting previous builds. We can scrap it."
  The player card's GAME LOG tab went with it (`playerSeasonLog` is no longer
  called on native), and `genSeasonLog.mjs` now writes only `public/pbp/`,
  the web's copy.
- **The demo board is gone (~500 KB of week-8 data).** That covers the screen,
  its Settings row, `assets/pbp/w8.json`, `assets/gamefeed/w8.json`, and
  `scripts/pbp/pick-demo-matchup.mts`. check:settingsmenu now asserts the row
  is gone.
- **Time zones trimmed (794 KB → 103 KB).** `apps/mobile/src/intl-tz-data.js`
  is generated by `scripts/gen-tz-data.mjs` from formatjs's `add-golden-tz`.
  It keeps every zone, because the phone's own zone must be present or times
  show in UTC, and trims each zone's history to 2020–2040. Verified identical
  to the golden data on 210,824 formatted times (all 246 zones, 2020–2040,
  including DST edges). Re-run the script after a formatjs upgrade.
- **Card backs and the loading splash are WebP** (1.11 MB → 0.68 MB, same
  dimensions, no visible difference).
- **Not done, on purpose:** compressing the bundle itself, which would save
  ~2.5 MB but slow every launch. Dropping the Intl polyfill entirely (~0.65 MB
  more) needs a device test of Hermes's own Intl first.
