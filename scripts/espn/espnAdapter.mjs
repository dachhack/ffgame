...
export function playToRows(p, roster, eventId, gameStartMs) {
  ...
  // Kicker — FG (own play) + XP (rides inside a TD play's text).
  if (typeText.startsWith('Field Goal')) {
    const m = reFG.exec(text);
    if (m && offTeam) out.push({ slug: `${offTeam.toLowerCase()}-k`, play: row(c, ride, /GOOD/i.test(text) ? 'fg' : 'fgmiss', Number(m[1]) || 0, 0, 0, 0, 0) });
  }
  if (/extra point/i.test(text) && offTeam) {
    out.push({ slug: `${offTeam.toLowerCase()}-k`, play: row(c, ride, /extra point is GOOD/i.test(text) ? 'xp' : 'xpmiss', 0, 0, 0, 0, 0) });
  }

  // Team defense — sack / INT / fumble recovery / def TD / safety, keyed by the
  ...
  if (defTeam) {
    ...
    if (typeText === 'Sack') out.push({ slug: d, play: row(c, ride, 'sack', 0, 0, 0, 0, 0) });
    if (/INTERCEPT/i.test(text) || typeText.includes('Interception')) out.push({ slug: d, play: row(c, ride, 'int', 0, 0, 0, 0, 0) });
    if (p?.isTurnover && /FUMBLE/i.test(text) && /RECOVERED by/i.test(text)) out.push({ slug: d, play: row(c, ride, 'fumrec', 0, 0, 0, 0, 0) });
    if (/SAFETY/i.test(text)) out.push({ slug: d, play: row(c, ride, 'safety', 0, 0, 0, 0, 0) });
  }
  return out;
}
