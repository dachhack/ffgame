...
// ── Emit ──
const out = `// AUTO-GENERATED by scripts/pbp/genRealPbp.mjs — do not edit by hand.
// Real 2025 play-by-play, all 14 weeks (Stathead MCP / nflverse), keyed by player id.
// Attribution by nflverse gsis_id; c=game-elapsed seconds, k=kind, y=yards, td/ca/tg=flags.
export interface RealPlay { c: number; k: 'pass' | 'rush' | 'rec' | 'incomplete'; y: number; td: number; ca: number; tg: number; }
export const REAL_WEEKS = new Set<number>([${WEEKS.join(', ')}]);
export const REAL_PBP: Record<number, Record<string, RealPlay[]>> = ${JSON.stringify(pbp)};
export const REAL_POINTS: Record<number, Record<string, number>> = ${JSON.stringify(points)};
`;
writeFileSync(new URL('../../src/data/realPbp.ts', here), out);
