...
import { chatUnread, leagueTrades, myMatchup, defaultOpenWeek } from '@drip/core/data/liveApi';
...
interface LeagueSignals { alarm: LineupAlarm | null; offers: number; }
...
export function useLeagueSignals(e: {
  ...
  const [s, setS] = useState<LeagueSignals>({ alarm: null, offers: 0 });
  useEffect(() => {
    let dead = false;
    const poll = async () => {
...
export function CardSignalPills(props: Parameters<typeof useLeagueSignals>[0]) {
  const t = useTheme();
  const { alarm, offers } = useLeagueSignals(props);
  if (!alarm && offers === 0) return null;
  return (
    <>
      {alarm && (
