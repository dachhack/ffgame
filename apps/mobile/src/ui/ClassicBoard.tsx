 ...
 *  slate with info and stats? In a chip you can select."
 *
 *  It is a FULL-WIDTH row rather than the literal centre column, because that
 *  column is ~90px between two team heads and a slate does not fit in it. The
 *  centre keeps what it is for (LIVE, the golf marker); the emptiness the card
 *  actually had is the band under the two totals, and that is what this fills.
 *
 *  Each chip carries what a generic scoreboard cannot: how many of THIS
 *  matchup's starters are in that game and what they are worth. A game neither
 *  side is in is dimmed rather than dropped — it was asked for as the week's
 *  slate, and hiding games would misrepresent the week. */
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, paddingRight: 8 }}>
        {chips.map((c) => {
          ...
          return (
            <Pressable
              key={c.key}
              onPress={() => { tap(); onSel(on ? null : c.key); }}
              style={{
            ...
            : (
              <View style={{ flexDirection: 'row', gap: 10 }}>
                {([['home', open.homePlayers], ['away', open.awayPlayers]] as const).map(([side, ps]) => (
                  <View key={side} style={{ flex: 1, minWidth: 0 }}>
                    <Mono size={7.5} tone="faint" style={{ marginBottom: 3 }}>
...
export function ClassicBoard({ userId, leagueId, rosterId }: { userId: string; leagueId: string; rosterId: number }) {
  ...
  /** ── THE WEEK STEPPER (v0.299.1) ──────────────────────────────────────
   *  Weeks 1 … the last regular-season one, whose bound comes from the
   *  league's playoff start. Until that read lands the forward arrow won't
  ...
  const swipe = useRef(PanResponder.create({
    ...
    onPanResponderRelease: (_e, g) => {
      ...
      weekStep.current(g.dx < 0 ? 1 : -1);
    },
  })).current;
  // The responder is created ONCE (a fresh one mid-gesture cancels it), so the
  // handler reads the current step through a ref rather than closing over the
  // first render's copy.
  const weekStep = useRef(goWeek);
  weekStep.current = goWeek;
    ...
    <ScrollView contentContainerStyle={{ padding: 12, paddingBottom: 48, gap: 10 }}
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
        {/* THE WEEK IS NAVIGATION NOW (v0.299.1), not a status line: the game
            ...
            page, not facts about the game in front of you. The week stayed and
            earns its place by moving you through the season — arrows here, and
            a swipe anywhere on the board. */}
        {matchup && (
